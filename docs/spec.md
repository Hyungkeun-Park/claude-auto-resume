...
# v4 해결: /proc/$pid/cmdline 직접 읽기 + auto-resume 제외
...
`pgrep -x claude`로 정확히 `claude` 바이너리만 매칭한 후, `/proc/cmdline`에서 session ID 포함 + auto-resume 미포함 여부를 확인.
...
부모: 서브에이전트 결과 수집 시도 → API 호출 → rate limit 화면 (기다리기 선택)
  ...
  → daemon: 대기 → rate 회복 → /proc/cmdline으로 active 감지 → skip + archive
...
| ~~HIGH~~ | ~~pgrep 자기참조~~ | v4: `/proc/$pid/cmdline` 직접 읽기 + `pgrep -x claude`로 정확 매칭. `pgrep -af` 제거 |
...
11. ~~pgrep이 daemon 자신을 active로 오탐~~ → v4: `pgrep -x claude` + `/proc/cmdline` 직접 읽기로 해결 ✅
...
| pgrep 자기참조 오탐 | daemon cmdline에 `claude-auto-resume.sh SESSION_ID`가 포함 | v4: `pgrep -x claude` + `/proc/$pid/cmdline` 직접 읽기 |
| Resume 실패 시 파일 소실 | 파일 삭제가 resume 시도 전에 수행됨 | success/ 또는 failed/ 디렉토리로 아카이브 |
| 동일 세션 중복 daemon | hook이 여러 번 fire되면 같은 세션에 daemon 다중 spawn | daemon 시작 시 `/proc/cmdline`으로 기존 daemon kill |
...
### Hook 시뮬레이션 테스트 (55개 시나리오, 158 assertions)
...
|----------|-------|----------|
| Stop hook 기본 | T01-T04 | rate < 100%, 100% 생성, 중복 방지, stale cache |
| Prompt guard 기본 | T05-T10 | rate < 100%, 100% 스케줄, 중복 방지, no cache |
| StopFailure 기본 | T11-T17 | rate < 100%, 100% 생성, 중복, stale, no cache, both 100% |
| Stop hook 상태머신 | T18-T25 | rate 회복 시 삭제+pkill, 동일 세션 prompt 갱신, 다른 세션 유지 |
| 특수 문자 prompt | T26-T32 | 큰따옴표, 작은따옴표, 백슬래시, 줄바꿈, 한국어, JSON-in-prompt, 혼합 |
| Atomic write | T33-T35 | .tmp 파일 생성 확인, 원본 미오염 |
| 전체 라이프사이클 | T36-T40 | 생성→유지→삭제, guard→stop confirm, both 100% → later reset |
| 경계값 | T41-T45 | 8시간 초과 skip, 99.5→100 반올림, resume_at 갱신 |
| 에러 회복 | T46-T50 | readonly dir, 빈 session_id, 빈 CWD, null 값 |
| Corrupted file | T51-T55 | 손상된 JSON, 빈 파일, null session_id, 부분 JSON, 99% 반올림 |
